...
pending = len(
    claims[claims["Status"]=="Pending"]
)

cancelled = len(
    claims[claims["Status"]=="Cancelled"]
)

completion_rate = round(
    (completed/total_claims)*100,
    1
)
...
if page=="🏠 Overview":
    ...
    with receivers to reduce food wastage
    and improve food accessibility.
    ...
    with col1:
        ...
        st.info(
            f"🏆 Top Food City : {food['Location'].mode()[0]}"
        )

        st.info(
            f"🍽 Most Common Meal : {food['Meal_Type'].mode()[0]}"
        )

    with col2:

        st.info(
            f"🏢 Top Provider : {food['Provider_Type'].mode()[0]}"
        )
